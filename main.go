...
import (
	dpfm_api_caller "data-platform-function-csv-generator-from-json-rmq-kube/DPFM_API_Caller"
...
func callProcess(rmq *rabbitmq.RabbitmqClient, caller *dpfm_api_caller.DPFMAPICaller, conf *config.Conf, msg rabbitmq.RabbitmqMessage) (err error) {
	...
	res, mountPath, errs := caller.AsyncCsvCreates(accepter, &input, &output, l, conf)
